...
import { trackPendingRequest, appendRequestLog, saveRequestDetail } from "@/lib/db";
 ...
 */
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, authContext, ccFilterNaming, rtkEnabled, rtkTeamId, rtkGetDb, cavemanEnabled, cavemanLevel, sourceFormatOverride, providerThinking }) {
  ...
  // RTK: compress tool_result content (multi-tenant pool mode)
  const rtkStats = await compressMessages(translatedBody, rtkEnabled, { teamId: rtkTeamId, getDb: rtkGetDb });
  ...
  const sharedCtx = { provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, authContext, clientRawRequest, onRequestSuccess };
