...
Devlens is a centralized B2B platform for provisioning and monitoring AI access across developer teams. Built on Next.js, it provides an OpenAI-compatible endpoint (`/v1/*`) and routes traffic across multiple upstream providers with translation, fallback, token refresh, and usage tracking.
...
- `GET /api/settings`
- `GET /api/v1/models`
- CLI target base URL should be `http://<host>:20261/v1` when `PORT=20261`
