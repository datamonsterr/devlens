...
async function reorderInTx(db, providerId, teamId) {
  const list = teamId != null
    ? (await db.all(`SELECT * FROM providerConnections WHERE provider = ? AND teamId = ?`, [providerId, teamId])).map(rowToConn)
    : (await db.all(`SELECT * FROM providerConnections WHERE provider = ?`, [providerId])).map(rowToConn);
  list.sort((a, b) => {
...
export async function createProviderConnection(data) {
  ...
  await db.transaction(async () => {
    const all = data.teamId != null
      ? (await db.all(`SELECT * FROM providerConnections WHERE provider = ? AND teamId = ?`, [data.provider, data.teamId])).map(rowToConn)
      : (await db.all(`SELECT * FROM providerConnections WHERE provider = ?`, [data.provider])).map(rowToConn);
    ...
    await reorderInTx(db, data.provider, data.teamId);
    result = conn;
  });
...
export async function updateProviderConnection(id, data, teamId) {
  ...
  await db.transaction(async () => {
    ...
    await upsert(db, merged);
    if (data.priority !== undefined) await reorderInTx(db, existing.provider, existing.teamId);
    result = merged;
  });
  return result;
}
...
export async function deleteProviderConnectionsByProvider(providerId, teamId) {
  ...
  const before = teamId != null
    ? await db.get(`SELECT COUNT(*) AS n FROM providerConnections WHERE provider = ? AND teamId = ?`, [providerId, teamId])
    : await db.get(`SELECT COUNT(*) AS n FROM providerConnections WHERE provider = ?`, [providerId]);
  if (teamId != null) {
    await db.run(`DELETE FROM providerConnections WHERE provider = ? AND teamId = ?`, [providerId, teamId]);
  } else {
...
export async function reorderProviderConnections(providerId, teamId) {
  const db = await getAdapter();
  await db.transaction(async () => await reorderInTx(db, providerId, teamId));
}
