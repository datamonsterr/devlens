...
export default function DefaultToolCard({ toolId, tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders = [], cloudEnabled = false, tunnelEnabled = false }) {
  ...
  const replaceVars = (text) => {
    ...
    // Add /api/v1 suffix only if not already present (DRY - avoid duplicate)
    const normalizedBaseUrl = baseUrl || "http://localhost:20261";
    const baseUrlWithV1 = normalizedBaseUrl.endsWith("/api/v1") 
      ? normalizedBaseUrl 
