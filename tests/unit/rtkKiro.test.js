...
describe("Kiro format RTK support", () => {
  it("compresses tool results in Kiro conversationState.currentMessage", async () => {
    const kiroBody = {
      conversationState: {
    ...
    const stats = await comp(kiroBody);
  ...
  it("compresses tool results in Kiro conversationState.history", async () => {
  ...
  it("handles multiple tool results across history and currentMessage", async () => {
  ...
  it("preserves error tool results without compression", async () => {
    const kiroBody = {
      conversationState: {
        chatTriggerType: "MANUAL",
  ...
  it("returns null when RTK is disabled", async () => {
    const kiroBody = {
      conversationState: {
        chatTriggerType: "MANUAL",
    ...
    const stats = await comp(kiroBody, false); // RTK disabled
    expect(stats).toBeNull();
  });

  it("handles Kiro body with no tool results gracefully", async () => {
    const kiroBody = {
      conversationState: {
        chatTriggerType: "MANUAL",
  ...
  it("handles malformed Kiro body without crashing", async () => {
    ...
    for (const body of malformedBodies) {
      const stats = await comp(body, true);
