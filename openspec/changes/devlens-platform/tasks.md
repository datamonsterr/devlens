...
- [ ] 6.10 Add pricing management UI: auto-fetched provider prices displayed, manager can override per-model with custom prices
...
- [ ] 9.3 Update Tailwind config with blue-purple primary palette
- [ ] 9.4 Update logo, favicon, and brand assets (if any exist in `public/`)
- [ ] 9.5 Update page titles, meta tags, and manifest references
...
- [ ] 13.1 Initialize `.opencode/` configuration with project-specific context and conventions
- [ ] 13.2 Install `skill-creator` skill via `npx skills install skill-creator`
- [ ] 13.3 Install `find-skills` skill via `npx skills install find-skills`
- [ ] 13.4 Use find-skills to locate and install `agent-browser` and related skills
- [ ] 13.5 Create subagent for writing unit tests: reads source, writes test file, runs test, reports results
- [ ] 13.6 Create subagent for running unit tests: executes vitest, captures failures, reports summary
- [ ] 13.7 Create subagent for debugging: adds log statements, reruns test, reads log output, fixes, loops until pass
- [ ] 13.8 Configure subagents to read/update `docs/` before opening new OpenSpec changes and after finishing implementation

## 14. Tests

- [ ] 14.1 Delete tests for removed modules (CLI tools, MITM, cloud sync)
- [ ] 14.2 Update test imports referencing renamed/removed files, modules, or API routes
- [ ] 14.3 Update test assertions referencing 9Router brand names
- [ ] 14.4 Update test fixtures and mocks for Clerk auth (replace JWT/password mocks)
- [ ] 14.5 Add tests for `requireRole()` authorization utility
- [ ] 14.6 Add tests for API key creation, validation, and revocation
- [ ] 14.7 Add tests for team management endpoints (view, update, invite, remove developer with usage retention)
- [ ] 14.8 Add tests for usage dashboard API with team-scoped data
- [ ] 14.9 Run full test suite: `npm test` — verify all tests pass with zero failures

## 15. Final Verification

- [ ] 15.1 Run `npm run build` — verify clean build with no errors
- [ ] 15.2 Run lint: `npm run lint` (if lint script exists, else add one) — verify clean
- [ ] 15.3 Start dev server: verify Clerk sign-in flow, role-based UI rendering, model browser, API key creation, dashboard
- [ ] 15.4 Verify `/v1/models` returns models scoped to API key's team
- [ ] 15.5 Verify `/v1/chat/completions` works with API key auth and tracks usage
- [ ] 15.6 Verify RTK pool decrements on streaming requests
