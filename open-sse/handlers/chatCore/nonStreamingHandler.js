...
import { appendRequestLog, saveRequestDetail } from "@/lib/db";
 ...
 */
export async function handleNonStreamingResponse({ providerResponse, provider, model, sourceFormat, targetFormat, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, authContext, clientRawRequest, onRequestSuccess, reqLogger, toolNameMap, trackDone, appendLog }) {
  trackDone();
  ...
  saveUsageStats({ provider, model, tokens: usage, connectionId, apiKey, authContext, endpoint: clientRawRequest?.endpoint });
